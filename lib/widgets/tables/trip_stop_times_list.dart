...
class TripStopTimesList extends StatefulWidget {
  ...
  @override
  State<TripStopTimesList> createState() => _TripStopTimesListState();
}

class _TripStopTimesListState extends State<TripStopTimesList> {
  bool _showAllPassedStops = false;

  @override
  Widget build(BuildContext context) {
    int lastPassedIndex = -1;
    for (int i = 0; i < widget.stopTimes.length; i++) {
      if (TripDetailsUtils.isPassedStop(widget.stopTimes[i], widget.serviceDay)) {
        lastPassedIndex = i;
      }
    }

    final hasEarlierPassedStops = lastPassedIndex >= 1;
    final hiddenCount = lastPassedIndex;
    final visibleStopTimes = (!hasEarlierPassedStops || _showAllPassedStops)
        ? widget.stopTimes
        : widget.stopTimes.sublist(lastPassedIndex);
    ...
    return Column(
      ...
      children: [
        if (hasEarlierPassedStops)
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: AppSpacing.md, vertical: AppSpacing.xs),
            child: InkWell(
              onTap: () {
                setState(() {
                  _showAllPassedStops = !_showAllPassedStops;
                });
              },
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      _showAllPassedStops ? Icons.expand_less_rounded : Icons.expand_more_rounded,
                      size: 20,
                    ...
                    Text(
                      _showAllPassedStops
                          ? AppTexts.tripHidePassedStops
                          : AppTexts.tripShowPassedStops(hiddenCount),
                      style: TextStyle(
                        fontSize: 13,
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            return DataTable(
              ...
              rows: visibleStopTimes.map((stopTime) {
                ...
                final passedStop = TripDetailsUtils.isPassedStop(stopTime, widget.serviceDay);
                ...
                final String? customTooltipMsg = (scheduledArr != null && scheduledDep != null && scheduledArr != scheduledDep)
                    ? AppTexts.tripScheduledTimeRange(
                        TripDetailsUtils.formatEpoch(scheduledArr, widget.serviceDay),
                        TripDetailsUtils.formatEpoch(scheduledDep, widget.serviceDay),
                      )
                    : null;
